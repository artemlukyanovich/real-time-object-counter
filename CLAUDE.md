...
**Async video pipeline (`video.async_pipeline`, default true).** `VideoSource` decodes+resizes in a background thread feeding a `queue.Queue`, so decode overlaps with inference; `read() → (ret, frame)` is unchanged so `run()` is agnostic. `video.drop_policy` (`auto`/`block`/`drop`) sets behavior when inference can't keep up: `block` = backpressure, never drop (deterministic — files); `drop` = keep only the freshest frame, queue size 1 (low latency — live camera/drone); `auto` resolves to block for files, drop for live sources (`_resolve_drop`). End-of-stream uses a sentinel; reader-thread exceptions are re-raised in `read()`; `release()` stops+joins the thread. Threading (not multiprocessing) works because `cap.read`/`cv2.resize`/CUDA release the GIL. The toggle is kept for A/B benchmarking + fallback. `PerformanceMetrics.record_io_wait` (shown as `avg_io_wait_ms` in the summary) measures main-loop frame-wait time — the key sync-vs-async signal. See `docs/async_pipeline.md`.
