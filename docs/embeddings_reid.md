...
reid:
  ...
  update_interval: 3   # запускать пайплайн каждые N кадров
  min_track_age: 1     # кадров до регистрации нового объекта (1 = мгновенно)
```
...
**Настройка `update_interval` и `min_track_age`** (в `configs/default.yaml`, секция `reid`):

`update_interval` — как часто запускать embedding-пайплайн. `min_track_age` — фильтрует «мелькающие» объекты: пока трек не «переживёт» заданное число **видеокадров** (не вызовов пайплайна), он не попадает в `unique` / `active`. Если трек совпадает с уже известным объектом по эмбеддингу — порог не применяется, объект сопоставляется мгновенно.

`min_track_age` измеряется в реальных кадрах по `frame_idx`, независимо от `update_interval`. При `update_interval: 10` и `min_track_age: 8` объект подтвердится через 8 кадров (≈0.25 сек при 30 FPS), а не через 80.

| `min_track_age` | Поведение |
|---|---|
| `1` | Отключено — каждый трек сразу становится объектом (прежнее поведение) |
...
## Методы агрегации эмбеддингов

По умолчанию `ObjectMemory` представляет каждый объект **средним значением** всех накопленных эмбеддингов (`aggregation_method: "mean"`). Это безопасный вариант для MVP, но у него есть слабое место: простое усреднение **размывает идентичность**, если:

- объект сильно меняется по внешнему виду между кадрами,
- в буфер попали «плохие» crops (частичная окклюзия, засветка, очень маленький размер),
- embedding'и ранних кадров оказались шумными.

Для таких случаев реализованы три альтернативных метода.

### Сравнительная таблица

| Метод | Как работает | Плюсы | Минусы | Когда использовать |
|---|---|---|---|---|
| `mean` | Среднее всех хранимых эмбеддингов | Стабильность, устойчивость к шуму | Медленно адаптируется, шумные эмбеддинги влияют наравне со свежими | Объект не меняется; короткие сцены; старт |
| `ema` | Экспоненциальное скользящее среднее: новый эмбеддинг весит `alpha`, старое EMA — `1-alpha` | Адаптируется к смене внешности, не «забывает» историю резко | При плохом crop быстро портит представление; нужна калибровка `ema_alpha` | Объект постепенно меняет вид (другой ракурс, освещение); долгие сцены |
| `weighted` | Взвешенное среднее с экспоненциальным затуханием: старые эмбеддинги множатся на `weighted_decay` каждый шаг | Плавная адаптация; старые данные продолжают влиять, но слабее | Сложнее подобрать `weighted_decay` под конкретный сценарий | Промежуточный вариант между `mean` и `ema` |
| `recent` | Среднее только последних `recent_n` эмбеддингов | Быстрая реакция на изменение внешности; игнорирует «устаревшую» историю | Уязвим к плохим crops в последних кадрах | Объект сильно меняется; длинные треки с изменением угла обзора |

### Конфигурация

Все параметры находятся в `configs/embeddings/default.yaml`, секция `memory`:

```yaml
memory:
  aggregation_method: "mean"   # mean | ema | weighted | recent
  ema_alpha: 0.3               # только для ema:      0.2 (медленно) – 0.5 (быстро)
  recent_n: 3                  # только для recent:   <= max_embeddings_per_object
  weighted_decay: 0.7          # только для weighted: 0.5 (крутое затухание) – 0.95 (почти flat)
```
...
`ema_alpha = 0.3` означает: новый эмбеддинг вносит 30% в представление, накопленная история — 70%. Если объект меняет вид быстро, увеличьте до 0.4–0.5.
...
Используются только 3 последних эмбеддинга из rolling-буфера. Если буфер `max_embeddings_per_object: 5`, то 2 самых старых игнорируются полностью.
...
При буфере из 5 эмбеддингов `[e0, e1, e2, e3, e4]` (e0 — самый старый, e4 — новейший) веса будут пропорциональны `[0.7³, 0.7², 0.7¹, 0.7⁰ · 0.7, 1.0]`, нормализованные до суммы 1.

### Как выбрать метод

1. **Начните с `mean`** — это самый предсказуемый вариант для большинства сцен.
2. Если в логах `test_reid_integration` видите, что `continuous` scores нестабильны (разброс велик) или RE-ID перестаёт работать после нескольких минут — попробуйте `ema` с `ema_alpha: 0.3`.
3. Если объект часто меняет ракурс (например, человек разворачивается) и threshold приходится снижать ниже 0.70 — попробуйте `recent` с `recent_n: 3`.
4. При плохом качестве crops (мелкие объекты, частые окклюзии) `mean` или `weighted` предпочтительнее, чем `recent` — последний метод нечувствителен к истории и легче «ломается» от шумных кадров.

> После смены метода агрегации **перекалибруйте `similarity_threshold`** командой `test_reid_integration` — разные методы дают разный диапазон similarity scores.

---

## Выбор модели OpenCLIP

Все модели используют одинаковый интерфейс `ObjectEmbedder` и меняются только двумя полями в конфиге: `model_name` и `pretrained`.

### Сравнительная таблица

| Модель | Размер патча | Размерность эмбеддинга | Размер весов | Скорость (CPU) | Точность ReID |
|---|---|---|---|---|---|
| `ViT-B-32` | 32×32 | 512 | ~350 МБ | быстро | базовая |
| `ViT-B-16` | 16×16 | 512 | ~350 МБ | средне | выше B-32 |
| `ViT-L-14` | 14×14 | 768 | ~890 МБ | медленно | высокая |
| `ViT-H-14` | 14×14 | 1024 | ~2.5 ГБ | очень медленно | наивысшая |

> Меньший размер патча → больше токенов на изображение → выше детализация признаков, но и выше вычислительная нагрузка.

### ViT-B-32 (по умолчанию)

Оптимальный выбор для реального времени. 32×32 патчи дают грубые пространственные признаки, достаточные для ReID крупных объектов (люди, автомобили). Работает на CPU с приемлемой задержкой (~15–40 мс/кадр в зависимости от числа объектов).
...
**Когда использовать:** прототипирование, нет GPU, объекты достаточно крупные (> 80×80 px в кропе).

### ViT-B-16

Та же архитектура Base, но патчи 16×16 — вдвое больше токенов. Размерность эмбеддинга та же (512), но признаки захватывают более тонкие детали текстуры. Рекомендуется как первый апгрейд от B-32 без роста памяти.
...
**Когда использовать:** GPU доступен, объекты средней крупности, нужна улучшенная точность без увеличения размерности.

### ViT-L-14

Модель Large: 24 трансформерных слоя против 12 в Base. Размерность эмбеддинга — 768. Заметно лучше разделяет похожие объекты (например, люди в одинаковой одежде). На CPU даёт задержку 100–300 мс/кадр, на современном GPU — 5–15 мс.
...
**Когда использовать:** GPU обязателен, объекты трудно различимы визуально, требуется высокая точность ReID.

> Размерность 768 вместо 512: если вы сохраняете эмбеддинги в `.npy` или передаёте их во внешнюю систему, учтите изменение формата.

### ViT-H-14

Модель Huge: 32 трансформерных слоя, размерность эмбеддинга — 1024. Веса занимают ~2.5 ГБ VRAM. Практически не используется в реальном времени — применяется для офлайн-анализа или когда точность важнее скорости.
...
**Когда использовать:** офлайн-обработка записанного видео, максимальная точность, GPU с ≥ 8 ГБ VRAM.

### Как выбрать `pretrained`

Тег весов указывает, на каком датасете и с каким батчем обучена модель. **Тег привязан к конкретной архитектуре** — нельзя использовать тег от одной модели с другой.

Рекомендуемые теги для ReID:

| Модель | `pretrained` | Датасет |
|---|---|---|
...
Для просмотра всех доступных пар `(model_name, pretrained)`:
...
Для ReID рекомендуются веса `laion2b_*` — они обучены на 2 млрд изображений и дают лучшие визуальные признаки, чем `openai` (обученные только на 400M пар текст–изображение).

### Влияние на `similarity_threshold`

Разные модели дают разные диапазоны cosine similarity для одного объекта. После смены модели **обязательно** перекалибруйте порог командой:
...
| Модель | Рекомендуемый `similarity_threshold` |
|---|---|
| `ViT-B-32` | 0.75 |
| `ViT-B-16` | 0.78 |
| `ViT-L-14` | 0.80 |
...
| Метод | Описание |
|---|---|
| `__init__(cropper, embedder, memory, min_track_age)` | Принимает готовые экземпляры модулей; `min_track_age` — минимум кадров до регистрации нового объекта |
